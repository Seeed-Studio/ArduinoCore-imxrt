...
{
	  status_t status;
		status = FlexSPI_NorFlash_Erase_Sector(BOOTLOADER_FLEXSPI, BOOTLOADER_DATA_AREA * SECTOR_SIZE);
    if (status != kStatus_Success)
    {
       ...
       return ;
		...
		status = FlexSPI_NorFlash_Page_Program(BOOTLOADER_FLEXSPI, BOOTLOADER_DATA_AREA * SECTOR_SIZE,
                                            (void *)&devInfo, 40);
    if (status != kStatus_Success)
    {
